...
public class MainController {
    ...
    private void setupEntryTable(TableView<SparseEntry> table) {
        var cRow = new TableColumn<SparseEntry, Number>("row");
        cRow.setCellValueFactory(d -> d.getValue().rowProperty());
        cRow.setCellFactory(TextFieldTableCell.forTableColumn(new IntegerStringConverter()));
        cRow.setOnEditCommit(e -> e.getRowValue().setRow(e.getNewValue().intValue()));

        var cCol = new TableColumn<SparseEntry, Number>("col");
        cCol.setCellValueFactory(d -> d.getValue().colProperty());
        cCol.setCellFactory(TextFieldTableCell.forTableColumn(new IntegerStringConverter()));
        cCol.setOnEditCommit(e -> e.getRowValue().setCol(e.getNewValue().intValue()));

        var cVal = new TableColumn<SparseEntry, Number>("value");
        cVal.setCellValueFactory(d -> d.getValue().valueProperty());
        cVal.setCellFactory(TextFieldTableCell.forTableColumn(new DoubleStringConverter()));
        cVal.setOnEditCommit(e -> e.getRowValue().setValue(e.getNewValue().doubleValue()));
    ...
    // ===== Buttons (sad zovu service, ali fake) =====
